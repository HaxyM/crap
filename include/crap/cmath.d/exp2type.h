#ifndef CRAP_CMATH_EXP2TYPE
#define CRAP_CMATH_EXP2TYPE
...
namespace crap
{
 template <class> struct exp2Type;
}
...
#include "../ratio.d/exp2type.h"

namespace crap
{
 template <intmax_t Numerator, intmax_t Denominator>
	 struct exp2Type<std :: ratio<Numerator, Denominator> >
 {
  private:
  ...
  using value = typename exp2Type <valueRatio<std :: intmax_t, (origNegative ? '-' : '+'), origNum, origDen> > :: type;
 ...
 template <intmax_t Numerator, intmax_t Denominator>
	 constexpr typename exp2Type <std :: ratio<Numerator, Denominator> > :: type
	 exp2(std :: ratio<Numerator, Denominator>) noexcept;
}

template <intmax_t Numerator, intmax_t Denominator>
inline constexpr typename crap :: exp2Type <std :: ratio<Numerator, Denominator> > :: type
crap :: exp2(std :: ratio<Numerator, Denominator>) noexcept
{
 return {};
}
#endif
