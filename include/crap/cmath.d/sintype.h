...
#include "../ratio.d/sintype.h"

namespace crap
{
 template <intmax_t Numerator, intmax_t Denominator>
	 struct sinType<std :: ratio<Numerator, Denominator> >
 {
  private:
  ...
  using value = typename sinType <valueRatio<std :: intmax_t, (origNegative ? '-' : '+'), origNum, origDen> > :: type;
