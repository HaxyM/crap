...
#ifndef CRAP_RATIO_LGAMMATYPE
#define CRAP_RATIO_LGAMMATYPE

namespace crap
{
 template <class Type, char Sign, Type Numerator, Type Denominator>
	 struct lgammaType<valueRatio<Type, Sign, Numerator, Denominator> >
 {
  private:
 ...
 struct lgammaType <valueRatio<Type, Sign, Numerator, Denominator> > :: template
	 ...
	 simpleFactorial<false, integralPart, fracPart>
 {
  ...
  public:
  using type = typename plusType <factorialPartResult, SpougesPartResult> :: type;
 };
	 ...
	 Implementation<true, Empty...>
 {
  ...
  template <Type Num> using makeLog = typename logType <makeRatio<Num> > :: type;
  template <Type ... Nums> using sum = typename plusType <makeLog<Nums>...> :: type;
  public:
 ...
 {
  using type = typename zero <integralPart> :: type;
 };
	 ...
	 factorialPart<true, false, Empty...>
 {
  ...
  template <Type Num> using makeLog = typename negateType <typename logType <makeElement<Num> > :: type> :: type;
  template <Type ... Nums> using sum = typename plusType <makeLog<Nums>...> :: type;
  public:
	 ...
	 factorialPart<false, true, Empty...>
 {
  ...
  template <Type Num> using makeLog = typename logType <makeElement<Num> > :: type;
  template <Type ... Nums> using sum = typename plusType <makeLog<Nums>...> :: type;
  public:
	 ...
	 SpougesPart<K, Sum, Param, A, true>
 {
  ...
  using logZpA = typename logType <zPlusA> :: type;
  using param1 = typename multipliesType <typename plusType <z, constHalf> :: type, logZpA> :: type;
  using param2 = typename negateType <zPlusA> :: type;
  using param3 = typename logType <Sum> :: type;
  public:
  using type = typename plusType <param1, param2, param3> :: type;
 };
 ...
 template <class Type, char Sign, Type Numerator, Type Denominator>
	 constexpr typename lgammaType <valueRatio<Type, Sign, Numerator, Denominator> > :: type
	 lgamma(valueRatio<Type, Sign, Numerator, Denominator>) noexcept;
}

template <class Type, char Sign, Type Numerator, Type Denominator>
inline constexpr typename crap :: lgammaType <crap :: valueRatio<Type, Sign, Numerator, Denominator> > :: type
crap :: lgamma(crap :: valueRatio<Type, Sign, Numerator, Denominator>) noexcept
{
 return {};
}
#endif
