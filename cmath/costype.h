...
namespace crap
{
	 ...
	 struct cosType<std :: ratio<Numerator, Denominator>, Steps>
 {
  ...
  template <std :: intmax_t N> using sumTerm = typename iotaValue <N, std :: intmax_t, 0> :: template type<product>;
  template <std :: intmax_t ... N> using sum = typename plusType <std :: ratio<1, 1>, sumTerm<N>...> :: type;
  public:
  using type = typename iotaValue <Steps, std :: intmax_t, 1> :: template type<sum>;
 };
