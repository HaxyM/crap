#include "../include/crap/cmath.d/trunctype.h"
#include "../include/crap/ratio.d/trunctype.h"
...
int main()
{
 static_assert(!(isCompleteType <crap :: truncType<void> > :: value), "No logical sense for operation on type that does not represent any value.");

 static_assert(std :: is_same<typename crap :: truncType <std :: ratio<-1, 2> > :: type, std :: ratio<0, 1> >(), "Trunc of -1/2 should be -1.");
 static_assert(std :: is_same<typename crap :: truncType <std :: ratio<1, 2> > :: type, std :: ratio<0, 1> >(), "Trunc of 1/2 should be 0.");
 static_assert(std :: is_same<typename crap :: truncType <std :: ratio<0, 1> > :: type, std :: ratio<0, 1> >(), "Trunc of 0 should be 0.");
 ...
 static_assert(std :: is_same<typename crap :: truncType <std :: ratio<(std :: numeric_limits <std :: intmax_t> :: min() + 1), (std :: numeric_limits <std :: intmax_t> :: min() + 1)> > :: type, std :: ratio<1, 1> >(), "Should not fail on close to end of range values.");
 static_assert(std :: is_same<typename crap :: truncType <std :: ratio<(std :: numeric_limits <std :: intmax_t> :: min() + 1), std :: numeric_limits <std :: intmax_t> :: max()> > :: type, std :: ratio<-1, 1> >(), "Should not fail on close to end of range values.");
 static_assert(std :: is_same<typename crap :: truncType <std :: ratio<std :: numeric_limits <std :: intmax_t> :: max(), (std :: numeric_limits <std :: intmax_t> :: min() + 1)> > :: type, std :: ratio<-1, 1> >(), "Should not fail on close to end of range values.");
 static_assert(std :: is_same<typename crap :: truncType <std :: ratio<1, (std :: numeric_limits <std :: intmax_t> :: min() + 1)> > :: type, std :: ratio<0, 1> >(), "Should not fail on close to end of range values.");
 //XXX: Though those are fine. Strange :(
 static_assert(std :: is_same<typename crap :: truncType <std :: ratio<std :: numeric_limits <std :: intmax_t> :: max(), 1> > :: type, std :: ratio<std :: numeric_limits <std :: intmax_t> :: max(), 1> >(), "Should not fail on close to end of range values.");
